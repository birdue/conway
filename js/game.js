class Game {
  ...
  _getActiveChunkXY(view) {
    return Math2D.globalToChunkXY(
      Math2D.screenToGlobal([this._halfWidth, this._halfHeight], view),
      this._config
    );
  }
  ...
  tick() {
    ...
    for (let x = -sd; x <= sd; x++) {
      for (let y = -sd; y <= sd; y++) {
        chunks[x + sd][y + sd] = this._getChunk(Math2D.add(active, [x, y]));
      }
    }
  ...
  render(view) {
    ...
    for (let xDelta = -rd; xDelta <= rd; xDelta++) {
      for (let yDelta = -rd; yDelta <= rd; yDelta++) {
        const [x, y] = Math2D.add(active, [xDelta, yDelta]);
        this._getChunk([x, y]).render([x, y], view, config, ctx);
      }
    }

    const [xTopLeft, yTopLeft] = Math2D.add(
      Math2D.multiply(
        Math2D.subtract(active, [rd, rd]),
  ...
  setCellAt([xScreen, yScreen], value, view) {
    ...
    const global = Math2D.screenToGlobal([xScreen, yScreen], view);
    const chunkXY = Math2D.globalToChunkXY(global, config);
    const local = Math2D.subtract(
      global,
      Math2D.getTopLeftOfChunk(chunkXY, config)
    );
    const cell = Math2D.localToCell(local, config);
