class Game {
	...
	tick() {
		...
		const chunks = Array(1 + 2*sd).fill(0).map(() => []);
		...
		for(let x = -sd; x <= sd; x++) {
			for(let y = -sd; y <= sd; y++) {
				// console.log(`Computing adjacent array for x,y=${x},${y}`)
				const adjs = Array(3).fill(0).map(() => []);
				// let deps=0;
				for(let xDelta = -1; xDelta <= 1; xDelta++) {
					...
					for(let yDelta = -1; yDelta <= 1; yDelta++) {
						...
						// console.log(`adj[${xDelta+1}][${yDelta+1}] is to be set to the chunk[${x+xDelta+sd}][${y+yDelta+sd}]`)
						adjs[xDelta+1][yDelta+1] = chunks[x+xDelta+sd][y+yDelta+sd];
						// deps++;
					}
				}
				// console.log(`${x},${y} has ${deps} dependencies`);
				// console.log(adjs[1][1] === chunks[x+sd][y+sd])
				// console.log(`for ${x},${y}, adjs is `, adjs)
				// console.log(`${xActive+x},${yActive+y} will be updated`)
				stasisReached = chunks[x+sd][y+sd].computeNextTick(adjs) && stasisReached;
			}
		}
	...
	render(view) {
		...
		const ctx = this._ctx, config = this._config;
		...
		for(let xDelta = -rd; xDelta <= rd; xDelta++) {
			for(let yDelta = -rd; yDelta <= rd; yDelta++) {
				...
				this._getChunk([x, y])
					.render([x, y], view, config, ctx);
			}
		}
		
		const [xTopLeft, yTopLeft] = add(
			multiply(
				subtract(active, [rd, rd]), Chunk.SIZE*cellSizeScreen
			), [xPan, yPan]
		)
		...
		for(let x = 0; x <= Chunk.SIZE*(1 + 2*rd); x++) {
			...
			ctx.lineTo(xScreen, yTopLeft + Chunk.SIZE*(1 + 2*rd)*cellSizeScreen);
		}
		for(let y = 0; y <= Chunk.SIZE*(1 + 2*rd); y++) {
			const yScreen = yTopLeft + y*cellSizeScreen;
			ctx.moveTo(xTopLeft, yScreen);
			ctx.lineTo(xTopLeft + Chunk.SIZE*(1 + 2*rd)*cellSizeScreen, yScreen);
		}
		ctx.stroke();
	}
